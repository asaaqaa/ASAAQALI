...
async def set_group_photo(gpic):
    ...
    if not gpic.is_group:
        await eor(
            gpic,
            "** ⪼ لا اعتقد ان ۿذه مجمـوعه 𓆰،**\n ⫷  𝙎𝙊𝙐𝙍𝘾𝞝  B𝞝R الأمبراطور ](t.me/kapo00s) ⫸",
        )
        return
...
async def promote(promt):
    ...
    icse = await eor(promt, "**╮ ❐  جـاري ࢪفع الورع مشرف  ❏╰**")
    ...
    try:
        ...
        await icse.edit("**- ❝ ⌊ تم تـرقيت الورع  مشـرف 𓆰.**")
        ...
        try:
            ...
            if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None:
                if chat.admin_rights.delete_messages is not True:
                    return await eor(
                        event,
                        "**⪼ لا تحاول لا يمكنك كتم أي شخص إذا لم يكن لديك إذن حذف الرسائل  𓆰،**",
                    )
...
async def kick(usr):
    ...
    if reason:
        ...
        await icse.edit(
            f"**- ❝ ⌊  تم طرد البزر ** [{user.first_name}](tg://user?id={user.id})  𓆰."
        )
    if BOTLOG:
